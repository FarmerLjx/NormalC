...
int main(void)
{
    ...
    unsigned char v1 = 0xd9;
    unsigned char v2 = 0;

    v2 = BitsReverse8(v1);

    for(i=0; i < (sizeof(FONT6x8) / sizeof(FONT6x8[0]) ); ++i){
		// 以16进制显示转换后的结果，指定为两位，自动在前面补零
        printf("0x%02x, ", BitsReverse8(FONT6x8[i]));
        // 每6个显示在一行
		if(i%6 == 5){
            if(5 == i){
                printf(" // \" \"\n");
